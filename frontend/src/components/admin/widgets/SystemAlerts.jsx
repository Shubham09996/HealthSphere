    ...
    <div className="bg-card p-6 rounded-xl border border-border/70 shadow-sm">
        ...
        <div className="space-y-3">
            {adminData.systemAlerts.map(alert => (
                 <div key={alert.id} className={`p-3 rounded-lg flex items-start gap-3 ${alert.severity === 'High' ? 'bg-red-500/10' : 'bg-orange-500/10'}`}>
                     <AlertTriangle className={`mt-0.5 flex-shrink-0 ${alert.severity === 'High' ? 'text-red-500' : 'text-orange-500'}`} size={16}/>
                    <div>
                         <p className="font-semibold text-foreground text-sm">{alert.type} Warning</p>
